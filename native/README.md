...
- Unity-style orientation navigation: X/Y/Z arms snap direction without changing projection; the center cube and `Persp`/`Iso` label toggle projection, and Shift-clicking the center restores the default perspective view. Free canvas orbit, gizmo orbit, pan, and wheel zoom preserve the selected projection.
