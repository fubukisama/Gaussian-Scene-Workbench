...
- Automatic Gaussian/point/mesh mode selection, deterministic large-scene sampling, scene-bounds camera fitting, and a manual diagnostic fallback. PLY polygon meshes support ASCII and binary little-/big-endian data, fan-triangulate indexed faces, compute smooth vertex normals, and render as lit double-sided surfaces while preserving point-only and Gaussian behavior.
