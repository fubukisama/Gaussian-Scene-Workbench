...
- A procedural infinite reference grid with adaptive decimal spacing, anti-aliasing, horizon/distance fading, and colored axes. Its default reference plane follows the imported model's lower bound so an offset model is not left floating tens or thousands of units above a hard-coded world origin; View > Reference Plane Grid can switch back to the exact world-zero plane. All six axis-aligned orthographic views select the matching screen-parallel XY, XZ, or YZ plane. Infinite axes use framebuffer-space line equations so dense small-scale grids do not turn shallow red/blue lines into comb-shaped fragments. The finite three-axis marker follows the chosen plane and scales with the scene radius. Its camera-relative visual range is independent of the tighter point/splat depth buffer, so zooming out does not expose a black cutoff. Presentation is synchronized to the display by default to prevent camera-motion tearing; `GSW_SWAP_INTERVAL=0` is an explicit benchmark-only override. The grid is viewport-only and cannot be selected, transformed, or saved as scene content.
