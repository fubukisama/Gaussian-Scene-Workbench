...
- Multi-object PLY import: importing into a populated scene asks whether to append (default), replace only the active object, or cancel. Point clouds, indexed meshes and Gaussian models can coexist in one depth-tested viewport, retain independent transforms, and be selected through the scene tree or visible geometry. Project Save/Save As, pending data migration, snapshots and untitled recovery retain the complete object collection; legacy single-scene projects still load. See [multi-scene import](../docs/MULTI_SCENE_IMPORT.md) for behavior and limits.
...
- Unity-style orientation navigation: the center is a real camera-projected, multi-face cube rather than a flat square; X/Y/Z use shaded cone heads, rear directions fade to neutral gray, and nearly view-aligned axes collapse instead of covering another control. The outer footprint remains compact while the center cube receives a larger circular hit target independent of its visual size; direction cones use the previously empty outer ring and enforce a safe gap from that target, so center projection clicks and axis snaps do not compete. Hover feedback reveals the center target without adding permanent chrome. Direction cones snap the view without changing projection; the center cube and `≡ Persp`/`≡ Iso` label toggle projection, and Shift-clicking the center restores the default perspective view. Free canvas orbit, gizmo orbit, pan, and wheel zoom preserve the selected projection.
...
- A camera-relative infinite reference grid with adaptive decimal spacing, 4x-MSAA line primitives, level-edge fading, and colored axes. Its default reference plane follows the imported model's lower bound so an offset model is not left floating tens or thousands of units above a hard-coded world origin; View > Reference Plane Grid can switch back to the exact world-zero plane. All six axis-aligned orthographic views select the matching screen-parallel XY, XZ, or YZ plane, while toggling an oblique camera between perspective and orthographic projection keeps the XY ground plane stable. Grid levels and axes are emitted as real line geometry instead of a periodic fullscreen texture, so shallow perspective lines remain continuous rather than turning into comb-shaped fragments or moire. The colored axes share the major grid's camera-relative center, visible extent, and edge fade, preventing a coplanar axis from appearing to rise past the plane near the horizon. The central reference marker follows the chosen plane with fixed scene-space shaft lengths and exact perspective/orthographic projection. There is no pixel-length floor, cap, or saturation curve: close-up endpoints continue moving outward and are clipped normally if offscreen. Only stroke width and endpoint artwork use readable screen sizes; distant cramped labels are omitted without stretching the axes. Bevel-highlighted shafts, light/shadow arrow faces, outlined XYZ lettering, and a small origin collar distinguish it from the subdued long grid axes. Triangle strokes preserve line width on drivers that clamp GL lines; every element retains scene depth so models hide rear portions. Axis length stays fixed in source units during camera navigation; numerical scale remains labeled on the grid. The infinite background grid keeps independent clip depth, so zooming or panning far away does not expose a black cutoff. Presentation is synchronized to the display by default to prevent camera-motion tearing; `GSW_SWAP_INTERVAL=0` is an explicit benchmark-only override. The grid is viewport-only and cannot be selected, transformed, or saved as scene content.
