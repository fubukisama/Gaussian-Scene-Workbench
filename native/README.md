...
- A procedural infinite reference grid with adaptive decimal spacing, anti-aliasing, horizon/distance fading, and colored world axes. Perspective view keeps the world-fixed Z=0/XY ground plane; all six axis-aligned orthographic views select the matching screen-parallel XY, XZ, or YZ world plane, so front/back, left/right, and top/bottom views retain a Blender-style scale. The finite three-axis origin marker remains visible after a point/Gaussian scene or live-training preview is loaded and scales with the scene radius, preventing the Z axis from collapsing at large scene scales. Its camera-relative visual range is independent of the tighter point/splat depth buffer, so zooming out does not expose a black cutoff. It is viewport-only and cannot be selected, transformed, or saved as scene content.
...
- Automatic Gaussian/point/mesh mode selection, scene-bounds camera fitting, and a manual diagnostic fallback. Fixed-width binary point-only PLY files above the editable-scene threshold use an exact 16-byte XYZ/RGB representation, one-million-point CPU/GPU chunks, progressive GPU upload, chunk-bounds view rejection, and a 12-million-point interaction budget that returns to every visible source point when navigation stops. The source file is never simplified or rewritten; editing is disabled for this full-resolution large-scene path. PLY polygon meshes support ASCII and binary little-/big-endian data, fan-triangulate indexed faces, compute smooth vertex normals, and render as lit double-sided surfaces while preserving point-only and Gaussian behavior.
...
Blender's infinite-grid behavior is also used as a visual and algorithmic reference. The native viewport implementation was written independently for this project's Z-up coordinate system; no Blender source or shader code is copied into this MIT-licensed application.
