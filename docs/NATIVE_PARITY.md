...
| Dataset | Images and video import, metadata preservation | partial | Metashape-style Add Photos/Add Folder commands select media before any required save setup. Managed import recursively accepts photos/videos into the untitled or saved project, preserves source metadata and originals, extracts frames, reports structured progress, and publishes through a journaled staging transaction that recovers after cancellation, process failure, or reopening a project. Existing `images`/`input` plus `sparse/0` datasets can also be linked without copying. Import cleanup transactionally removes only managed dataset/reconstruction copies, detaches external datasets without changing their files, and can unload a scene without deleting its PLY/training output. Mask import and detailed per-file counters remain missing. |
...
| Reference grid | Infinite world grid and axes | available | A procedural OpenGL grid intersects the fixed Y=0/XZ world plane, adapts its decimal scale while zooming, and uses a camera-relative visual range independent of the scene depth buffer. It remains visible after a 5000 px pan and 24-notch zoom-out smoke, fades only near its distant horizon, and never enters selection, transforms, or project serialization. |
