...
def sources():
    ...
    for path in sorted((ROOT / 'native/src').glob('*')):
        if path.suffix not in {'.h', '.cpp'} or path.name in {'main.cpp', 'MultiSceneSmokeTest.cpp', 'LanguageSmokeTest.cpp', 'WindowUiSmokeTest.cpp', 'GaussianPerformanceSmokeTest.cpp', 'ObservationNavigationSmokeTest.cpp'}:
            continue
